...
write_files:
  - content: |
      ...
      vault_tls_ca_filename: ${vault_tls_ca_filename}
...
runcmd:
  ...
  - aws ssm get-parameter --region ${aws_region} --with-decryption --name ${ssm_parameter_tls_certificate}  | jq --raw-output '.Parameter.Value' > ${vault_tls_path}/${vault_tls_cert_filename}
  - aws ssm get-parameter --region ${aws_region} --with-decryption --name ${ssm_parameter_tls_key}   | jq --raw-output '.Parameter.Value' > ${vault_tls_path}/${vault_tls_key_filename}
  - aws ssm get-parameter --region ${aws_region} --with-decryption --name ${ssm_parameter_tls_ca}   | jq --raw-output '.Parameter.Value' > ${vault_tls_path}/${vault_tls_ca_filename}
  - aws ec2 describe-instances --region ${aws_region} --filters Name=tag:VaultRetryJoin,Values=${cluster_name} Name=instance-state-name,Values=pending,running | jq --raw-output '[.Reservations[].Instances[].PrivateDnsName] | map(rtrimstr(".${aws_region}.compute.internal")) | map(rtrimstr(".ec2.internal"))' > /etc/vault.d/setup/peer_nodes.json
